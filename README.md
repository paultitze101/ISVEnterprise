...
- More features will be added as required once the final design is ironed out (keeping mass, Δv budget, vehicle mass fraction etc in mind). For eg there is a possibility that a 500 m total diameter segmented adaptive optics mirror (for multiple station laser tracking) may be added to the rear of the front dust shield to use Photonic Laser Propulsion (PLP) for initial acceleration in the vicinity of the solar system from the high power (100 GW total fusion powered) laser transmitter array in addition to the 3 antiproton-beryllium-beam-core Matter Antimatter Propulsion (MAP) main engines and 4 antiproton-catalyzed-aneutronic Nuclear Fusion Propulsion (NFP) auxiliary engines. This would make ISV Enterprise a starship with a tri-modal propulsion system, however looking into the details and the numbers. Note for any beamed power propulsion system, although it offers a form of propellantless propulsion at the highest possible Isp via photon momentum transfer to the starship, it does setup a dependency on the transmitter base station and hence an extra risk to the mission if that base station fails and also assumes an extensive solar system infrastructure base with its significant associated power systems to power the laser arrays. <I>Negotiable</I>
