...
- One 40 m single stage fully reusable autonomous rescue lander (to be used if the main transport shuttle is stranded on the planetary surface) able to undock from the mother ship, land on a planetary surface without a pilot near the rescue beacon activated by the stranded crew, can seat 8 crew at a time then liftoff and dock back at the mother ship. It can also refuel LH₂ from the mother ship and go again if required with a short tunraround time. The craft is robust enough to handle various types of atmospheres and re-entry conditions including a range of surface gravity environments up to 1.5g however has a limited range enough to reach LEO and dock with the mother ship. Uses compact 4x Centrifugal Nuclear Thermal Rocket (CNTR molten core) engines Isp 1,800 s @1.1 MN each.<I>Compulsory</I> 
