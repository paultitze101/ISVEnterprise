...
- More features will be added as required once the final design is ironed out (keeping mass, Δv budget, vehicle mass fraction etc in mind). There is a possibility that a detachable ring composing of a split circular segmented mirror with top and bottom half assemblies may be temporarily added to the rear of the ship for Photonic Laser Propulsion (PLP) for initial acceleration and deceleration assist in the vicinity of the solar system from the very high power TW class laser transmitter arrays (2 will be used at a time). The segmented mirrors will point towards the rear of the ship 25° away from the MAP and NFP engine exhaust plumes for a clean line of sight to the PLP laser arrays stationed throughout the exit path out of the solar ystem. The ring detaches from the ship once we are past the PLP infrastructure in the solar system as it is no longer required and reduces overall mass of the ship. This is in addition to the 5 antiproton-beryllium-beam-core Matter Antimatter Propulsion (MAP) main engines (Isp 20,000,000 s @100 kN thrust) and 8 antiproton-catalyzed-aneutronic Nuclear Fusion Propulsion (NFP) auxiliary engines (Isp 400,000 s @500 kN thrust). This would make ISV Enterprise a starship with a tri-modal propulsion system when near the solar system, however looking into the details and the numbers. Note for any beamed power propulsion system, although it offers a form of propellantless propulsion at the highest possible Isp via photon momentum transfer to the starship, it does setup a dependency on the transmitter base stations and also assumes an extensive solar system wide infrastructure base with its significant associated power systems for the PLP laser arrays. <I>Negotiable</I>
